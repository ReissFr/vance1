// User-facing trigger: queue a receipts scan task and kick off the worker
// fire-and-forget. Returns the task_id so the UI can show a running indicator.
...
export async function POST(req: NextRequest) {
  ...
  const title = body.title ?? "Receipts scan";
  ...
  void fetch(`${baseUrl}/api/tasks/run-receipts-scan`, {
    ...
    console.warn("[receipts/scan] trigger fetch failed:", e);
  });
