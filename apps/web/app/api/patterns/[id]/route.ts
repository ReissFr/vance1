// PATCH /api/patterns/:id — respond to or annotate a pattern.
//   Bodies (mutually exclusive groups):
//     { status: "confirmed" | "contested" | "dismissed", user_note?: string }
//     { user_note: string }      — annotate without resolving
//     { pin: boolean }
//     { archive: true } / { restore: true }
//
// DELETE /api/patterns/:id — hard remove.
...
const VALID_STATUSES = new Set(["confirmed", "contested", "dismissed"]);
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (body.status != null) {
    if (typeof body.status !== "string" || !VALID_STATUSES.has(body.status)) {
      return NextResponse.json({ error: "status must be confirmed | contested | dismissed" }, { status: 400 });
    }
