// PATCH /api/used-to/:id — respond to or annotate a mined used-to statement.
//   Bodies (mutually exclusive groups):
//     { status: "reclaimed" | "grieved" | "let_go" | "noted" | "dismissed", status_note?: string }
//     { status_note: string }     — annotate without resolving
//     { pin: boolean }
//     { archive: true } / { restore: true }
//
//   When status='reclaimed', status_note MUST contain what the user is doing
//   (or has scheduled to do) to bring the lost self back. When status='grieved',
//   status_note (optional) holds the user's grief sentence. When status='let_go',
//   status_note (optional) holds the user's let-go reason.
//
// DELETE /api/used-to/:id — hard remove.
...
const VALID_STATUSES = new Set(["reclaimed", "grieved", "let_go", "noted", "dismissed"]);
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (body.status != null) {
    if (typeof body.status !== "string" || !VALID_STATUSES.has(body.status)) {
      return NextResponse.json({ error: "status must be reclaimed | grieved | let_go | noted | dismissed" }, { status: 400 });
    }
  ...
  if (reclaimingNow) {
    ...
    if (!note) {
      return NextResponse.json({ error: "status_note (what you did or scheduled to bring this back) is required when status='reclaimed'" }, { status: 400 });
    }
  }
  ...
  return NextResponse.json({ used_to: updated });
}
